...
#pragma omp parallel
    {
        vector<pair<int, int>> dist0_tmp;

#pragma omp for
        for (int i = 0; i < adj_0.size(); i++) {
            dist[adj_0[i].second] = dist[0] + adj_0[i].first;
            dist0_tmp.emplace_back(dist[adj_0[i].second], adj_0[i].second);
        }

        for (int i = 0; i < dist0_tmp.size(); i++) {
#pragma omp critical
            min_heap.emplace(dist[dist0_tmp[i].second], dist0_tmp[i].second);
        }
    }
