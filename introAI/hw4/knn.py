...
class FeatureProcessor:
    ...
    def create_interaction_features(row):
        """Create new features from interactions of existing ones"""
        new_features = []
        ...
        new_features.append(total_charges /
                            (tenure + 1))  # Add 1 to avoid division by zero

        # Service density (number of services per customer)
        services = [float(row[i]) for i in [6, 8, 9, 10, 11, 12, 13]
                    ]  # Service-related columns
        service_density = sum(1 for s in services if s > 0) / len(services)
        new_features.append(service_density)

        # Charges relative to tenure
        new_features.append(monthly_charges * math.log(tenure + 1))
...
class DataPreprocessor:
    ...
    def process(self):
        self.dataLabeling()
        self.addInteractionFeatures()
        self.dataStandardization()
        return self.dataset

    def addInteractionFeatures(self):
        ...
        for data in [
            ...
            for row in data:
                new_features = FeatureProcessor.create_interaction_features(
                    row)
                row.extend(new_features)
